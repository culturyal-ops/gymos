-- 005_vault_secrets_fallback_fixed.sql
...
do $$
begin
  if not exists (select 1 from pg_indexes where indexname = 'idx_gym_secrets_gym_id') then
    create index idx_gym_secrets_gym_id on public.gym_secrets(gym_id);
  end if;
end $$;

-- Enable RLS and drop/recreate policy to avoid conflicts
alter table public.gym_secrets enable row level security;

-- Drop existing policy if it exists, then create new one
drop policy if exists "Service role only access to secrets" on public.gym_secrets;
...
do $$
begin
  if not exists (select 1 from pg_indexes where indexname = 'idx_subscriptions_gym_id') then
...
do $$
begin
  if not exists (select 1 from pg_indexes where indexname = 'idx_invoices_gym_id') then
...
do $$
begin
  if not exists (select 1 from pg_indexes where indexname = 'idx_dlq_gym_id') then
    create index idx_dlq_gym_id on public.dead_letter_queue(gym_id);
  end if;
  if not exists (select 1 from pg_indexes where indexname = 'idx_dlq_created') then
    create index idx_dlq_created on public.dead_letter_queue(created_at desc);
  end if;
end $$;

alter table public.dead_letter_queue enable row level security;
drop policy if exists "No direct access to DLQ" on public.dead_letter_queue;
