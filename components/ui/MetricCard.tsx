...
export function MetricCard({
  ...
  return (
    <motion.article
      className={cn("card border-t-2 p-5", accentStyles[accentColor])}
    ...
    >
      ...
      <p className="mt-2 font-display text-[28px] font-bold leading-none">{value}</p>
      <div className="mt-3 flex items-center justify-between gap-2">
        <p className="text-xs text-[--color-text-secondary]">{subtext}</p>
        {badge ? (
          <span
            className={cn(
              "whitespace-nowrap rounded-full px-2 py-1 text-[11px] font-medium",
